...
namespace KPlugin.AppLovinMax.Editor
{
    ...
    public class SelectAdIdDrawer : PropertyDrawer
    {
        ...
        {
            AppLovinMaxSetting appLovinMaxSetting = AppLovinMaxSetting.Instance;
            if (appLovinMaxSetting == null)
                return new string[0];
            //
            SelectAdIdAttribute adIdAttribute = attribute as SelectAdIdAttribute;
            int count = appLovinMaxSetting.Ad_Count(adIdAttribute.Type);
            string[] adIds = new string[count];
            for (int i = 0; i < count; i++)
                adIds[i] = appLovinMaxSetting.Ad_Get(adIdAttribute.Type, i).AdID;
            return adIds;
        }
        #endregion
    }
